...
[English](README.en.md) · [项目详解](docs/INTRODUCTION.md) · [参与共创](CONTRIBUTING.md) · [提示词](prompts/README.md) · [知识库](knowledge/README.md) · [架构](docs/ARCHITECTURE.md)

给想做心理支持对话的人，一套可以读懂、修改和自行部署的后端。

有人下班后只想把烦心事说出来，有人被一段关系困住，想理清自己在意什么；也有人已经聊了很久，希望这次能往前走一小步。这些对话需要不同的节奏。这个项目保留了两种文字对话模式，也把上下文、记忆、安全判断和方法卡检索放在同一条链路里。
...
两种模式共享安全判断和记忆，但采用不同的主提示词。开发者按角色选择模型，不要求不同角色使用不同模型。
...
- `REPLY_API_KEY` / `REPLY_BASE_URL`：回复服务凭证和兼容端点。
- `SAFETY_API_KEY` / `SAFETY_BASE_URL`：安全判断服务凭证和兼容端点。
- `MEMORY_API_KEY` / `MEMORY_BASE_URL`：记忆提取服务凭证和兼容端点。
- `EMBEDDING_API_KEY`：模型凭证；同时填写 `EMBEDDING_BASE_URL`、`EMBEDDING_MODEL`。当前向量存储要求输出 1024 维，使用 OpenAI 兼容 embedding 接口。更换模型时需重建方法卡和事件向量，不能混用旧向量。
- `MEM0_OSS_DATA_DIR`：本机可写的绝对路径，用于本地 Qdrant 和 Mem0 历史数据；应在 Git 仓库外。

模型名在 `config/chat_clean_v1.json`，端点在 `.env`。三个角色的模型名默认为空，必须填写自己的模型 ID。聊天适配器使用 OpenAI 兼容的 Chat Completions 接口；模型还需支持本项目的 JSON 输出契约。其他协议需要自行接适配器，不能保证任意模型直接兼容。`claude` 和 `claude_coach` 是保留的模式标识，不限制模型品牌。
...
长期画像保存在 PostgreSQL；事件记忆使用 Mem0 OSS / Qdrant，embedding 发送给自行配置的服务。模型会接收本轮消息和相关上下文。**自行部署不等于数据完全离线。** 详情与删除边界见 [隐私说明](docs/PRIVACY.md)。
